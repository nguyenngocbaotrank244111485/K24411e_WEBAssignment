...
function calcDisplayPrice(sellerPrice) {
  return Math.ceil(sellerPrice * (1 + PLATFORM_CONFIG.feePercent / 100));
}
...
function saveTransaction(tx) {
  const txns = JSON.parse(localStorage.getItem(STORAGE_KEYS.txn) || '[]');
  txns.push(tx);
...
async function initMarketplace() {
  const session = JSON.parse(sessionStorage.getItem('printify_session') || 'null');
  if (!session) {
    showToast('❌ Bạn cần đăng nhập để dùng tính năng này!');
  ...
  const savedMarket = JSON.parse(localStorage.getItem(STORAGE_KEYS.market) || '[]');
  allListings = mergeListings(seedListings, savedMarket);
  allOrders = JSON.parse(localStorage.getItem(STORAGE_KEYS.orders) || '[]');
  allDesigns = JSON.parse(localStorage.getItem(STORAGE_KEYS.designs) || '[]');
...
// ── WATERMARK ──────────────────────────────────────────
// Nhúng userId + userName chéo giữa ảnh thiết kế
// Trả về Promise<base64string>
function addWatermark(thumbnailBase64, userId, userName) {
  return new Promise((resolve) => {
    ...
    img.onload = function() {
      ...
      // Vẽ watermark lặp lại 3×3 chéo 30°
...
// Load danh sách thiết kế của user vào dropdown
function loadMyDesigns() {
  const myDesigns = allDesigns.filter(d => d.userId === currentUser.userId);
  const sel = document.getElementById('design-select');
  sel.innerHTML = '<option value="">-- Chọn thiết kế --</option>' +
    myDesigns.map(d =>
...
// Tính và hiển thị giá display khi user nhập giá bán
...
let selectedOrderItem = null; // item đang được chọn để đăng bán

// Load các đơn hàng đã hoàn tất của user

function loadMyOrders() {
  let myOrders = allOrders.filter(o =>
    o.custId === currentUser.userId && o.status === 'Hoàn tất'
  );
...
// Chọn 1 item từ lịch sử để điền vào form đăng bán
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (qtyInput) {
    qtyInput.addEventListener('input', function() {
      ...
      // Luồng ngoại lệ: số lượng > số thực nhận
...
// Submit đăng bán sản phẩm in ấn
...
function seedDemoOrders() {
  ...
  localStorage.setItem('printify_orders', JSON.stringify(allOrders));
...
container.innerHTML = listings.map(l => {
  const remain = l.type === 'product' ? Math.max(0, (l.quantity || 0) - (l.soldQuantity || 0)) : null;

  return `
    <div class="listing-card" onclick="openListingDetail('${l.listingId}')">
      <div class="listing-img-wrap">
        <img src="${l.watermarkedBase64 || l.thumbnailBase64 || 'images/products/placeholder.png'}"
             alt="${l.title}" class="listing-img">
        <span class="listing-type-badge ${l.type === 'design' ? 'badge-design' : 'badge-product'}">
          ${l.type === 'design' ? '🎨 Thiết kế' : '📦 Sản phẩm'}
        </span>
        ${l.type === 'product' ? `<span class="listing-qty-badge">Còn ${remain}</span>` : ''}
      ...
      <div class="listing-info">
        <div class="listing-title">${l.title}</div>
        <div class="listing-tags">${(l.tags || []).slice(0, 3).map(t => `<span class="tag">${t}</span>`).join('')}</div>
        <div class="listing-price">${formatPrice(l.displayPrice)}</div>
        <div class="listing-seller">bởi ${getSellerName(l.sellerId)}</div>
      </div>
    </div>
...
function openListingDetail(listingId) {
  ...
  if (l.type === 'product') {
    const remain = l.quantity - l.soldQuantity;
  ...
  document.getElementById('modal-btn-editor').onclick = () => {
    closeModal();
    buyAndOpenEditor(l);
  };
...
function buyAndOpenEditor(listing) {
  ...
  const qty = listing.type === 'product'
    ? parseInt(document.getElementById('modal-qty-input').value) || 1 : 1;

  processPurchase(listing, qty);

  // Lưu designId vào sessionStorage để editor load lại
  sessionStorage.setItem('load_design_id', listing.designId);
  showToast('✅ Đã mua! Đang mở editor...');
  setTimeout(() => { window.location.href = 'editor.html?designId=' + listing.designId; }, 1200);
}
...
function buyAndAddToCart(listing) {
  ...
  const cartItem = {
    ...
    fromListing:     listing.listingId  // biết item mua từ marketplace
  };
...
function processPurchase(listing, qty) {
  ...
  if (updatedListing) {
    ...
    localStorage.setItem('printify_market', JSON.stringify(allListings));
  }
}
...
function reportListing(listingId) {
  ...
  // Luồng ngoại lệ: đủ báo cáo → tự ẩn + thông báo người bán
...
function removeListing(listingId) {
  ...
  allListings = JSON.parse(localStorage.getItem('printify_market') || '[]');
...
function renderMyListings() {
  const myListings = allListings.filter(l => l.sellerId === currentUser?.userId);
    ...
    <div class="my-listing-row">
      ...
      <div class="my-listing-info">
        ...
        <div class="my-listing-meta">
          <span class="status-badge status-${l.status.replace(/\s/g,'-')}">${l.status}</span>
          ${l.type === 'design'
